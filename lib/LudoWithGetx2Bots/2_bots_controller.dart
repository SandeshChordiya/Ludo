...
class LudoWith2BotsController extends GetxController {
  //
  ...
  LudoWith2BotsGameState _gameState = LudoWith2BotsGameState.throwDice;

  // Check any changes in _gamestate and save them
  LudoWith2BotsGameState get gameState => _gameState;

  // Which player is playing ?
  LudoWith2BotsPlayer _currentColor = LudoWith2BotsPlayer.red;

  // sum of red pawns
  int get sumRed {
    RxInt trialSum = 0.obs;
    for (int i = 0; i < player(LudoWith2BotsPlayer.red).pawns.length; i++) {
      trialSum.value += player(LudoWith2BotsPlayer.red).pawns[i].movement + 1;
    }
...
// sum of Yellow pawns
  int get sumYellow {
    RxInt trialSum = 0.obs;
    for (int i = 0; i < player(LudoWith2BotsPlayer.yellow).pawns.length; i++) {
      trialSum.value +=
          player(LudoWith2BotsPlayer.yellow).pawns[i].movement + 1;
    }
    return trialSum.value;
  }
  ...
  int get greenSum {
    RxInt trialSum = 0.obs;
    for (int i = 0; i < player(LudoWith2BotsPlayer.green).pawns.length; i++) {
      trialSum.value += player(LudoWith2BotsPlayer.green).pawns[i].movement + 1;
    }
    return trialSum.value;
  }
...
// For saving and using diceRoll values in widgets
  ...
  final List<LudoWith2BotsPlayers> players = [
    LudoWith2BotsPlayers(LudoWith2BotsPlayer.red),
    LudoWith2BotsPlayers(LudoWith2BotsPlayer.yellow),
    LudoWith2BotsPlayers(LudoWith2BotsPlayer.green),
  ];

  // To check the current player
  LudoWith2BotsPlayers get currentPlayer =>
  ...
  final List<LudoWith2BotsPlayer> won = [];

  // To collect and save the info on the current player
  LudoWith2BotsPlayers player(LudoWith2BotsPlayer col) =>
      players.firstWhere((element) => element.colr == col);

  void rollDice() async {
    // If Computer is playing then use delay to make it more realistic
    int delay = 0;
    if (currentPlayer.colr == LudoWith2BotsPlayer.yellow ||
        currentPlayer.colr == LudoWith2BotsPlayer.green) {
    ...
    Future.delayed(Duration(seconds: delay)).then((value) {
      ...
      Future.delayed(const Duration(seconds: 1)).then((value) {
        ...
        if (diceRoll == 6) {
          ...
          _gameState = LudoWith2BotsGameState.pickPawn;
        } else {
        ...
        if (currentPlayer.colr == LudoWith2BotsPlayer.red) {
        ...
        else if (currentPlayer.colr == LudoWith2BotsPlayer.yellow ||
            currentPlayer.colr == LudoWith2BotsPlayer.green) {
  ...
  void move(LudoWith2BotsPlayer co, int index, int movement) async {
    i.value = 0;
    forResponsive.value++;
    ...
    _gameState = LudoWith2BotsGameState.moving;
    ...
    for (int i = selectedPlayer.pawns[index].movement; i < movement; i++) {
      ...
      print("Audio Move Pawn");
      forResponsive.value++;
      if (_stopTravelling.value) break;
    }
    if (checkToKill(co, index, movement, selectedPlayer.location)) {
      forResponsive.value++;
      _gameState = LudoWith2BotsGameState.throwDice;
      if (currentPlayer.colr == LudoWith2BotsPlayer.yellow ||
          currentPlayer.colr == LudoWith2BotsPlayer.green) {
        rollDice();
      }
    ...
    await Future.delayed(Duration(seconds: 1)).then((value) {
      ...
      switch (_currentColor) {
        case LudoWith2BotsPlayer.red:
          ...
          _currentColor = LudoWith2BotsPlayer.yellow;
          break;
        case LudoWith2BotsPlayer.yellow:
          six.value = 0;
          _playerTurn.value = 3;
          _computerTurn.value = true;
          _currentColor = LudoWith2BotsPlayer.green;
          break;
        case LudoWith2BotsPlayer.green:
          six.value = 0;
          _playerTurn.value = 1;
          _computerTurn.value = false;
          _currentColor = LudoWith2BotsPlayer.red;
          break;
      }

      if (won.contains(currentPlayer.colr)) {
        _gameState = LudoWith2BotsGameState.finish;
        return;
      }
    });
    _gameState = LudoWith2BotsGameState.throwDice;
  ...
  bool checkToKill(
      LudoWith2BotsPlayer type, int index, int step, List<List<double>> path) {
    RxBool killSomeone = false.obs;
    for (int i = 0; i < 4; i++) {
      var redElement = player(LudoWith2BotsPlayer.red).pawns[i];
      var yellowElement = player(LudoWith2BotsPlayer.yellow).pawns[i];
      var greenElement = player(LudoWith2BotsPlayer.green).pawns[i];

      if ((redElement.movement > 0 &&
              !LudoWith2BotsPath.safeArea.map((e) => e.toString()).contains(
                  player(LudoWith2BotsPlayer.red)
                      .location[redElement.movement]
                      .toString())) &&
          type != LudoWith2BotsPlayer.red) {
        if (player(LudoWith2BotsPlayer.red)
                .location[redElement.movement]
                .toString() ==
            path[step - 1].toString()) {
          killSomeone.value = true;
          player(LudoWith2BotsPlayer.red).movePawn(i, 0);
        }
      }
      if ((yellowElement.movement > 0 &&
              !LudoWith2BotsPath.safeArea.map((e) => e.toString()).contains(
                  player(LudoWith2BotsPlayer.yellow)
                      .location[yellowElement.movement]
                      .toString())) &&
          type != LudoWith2BotsPlayer.yellow) {
        if (player(LudoWith2BotsPlayer.yellow)
                .location[yellowElement.movement]
                .toString() ==
            path[step - 1].toString()) {
          killSomeone.value = true;
          player(LudoWith2BotsPlayer.yellow).movePawn(i, 0);
        }
      }
      if ((greenElement.movement > 0 &&
              !LudoWith2BotsPath.safeArea.map((e) => e.toString()).contains(
                  player(LudoWith2BotsPlayer.green)
                      .location[greenElement.movement]
                      .toString())) &&
          type != LudoWith2BotsPlayer.green) {
        if (player(LudoWith2BotsPlayer.green)
                .location[greenElement.movement]
                .toString() ==
            path[step - 1].toString()) {
          killSomeone.value = true;
          player(LudoWith2BotsPlayer.green).movePawn(i, 0);
        }
      }
    }
  ...
  void validateWin(LudoWith2BotsPlayer color) {
    forResponsive.value++;
    if (player(color)
        .pawns
        .map((e) => e.movement)
    ...
    else if (calcLostMoves >= 3 && color == LudoWith2BotsPlayer.red) {
      if (sumYellow > greenSum) {
        won.add(LudoWith2BotsPlayer.yellow);
        won.add(LudoWith2BotsPlayer.green);
        won.add(LudoWith2BotsPlayer.red);
